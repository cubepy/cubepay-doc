...
- The `integrations/faoxima-ready-files/` guide documents a "replace these exact files at these exact paths in a specific third-party bot codebase" pattern. It separates **required/functional** files from **cosmetic/optional** ones (button text only) and calls out any accompanying SQL statement needed. Preserve that required-vs-cosmetic split when updating it or adding a new platform that still needs this pattern.
  - The **required** set is currently five files: `business_logic_1.php`, `successful.php`, `database_helpers_2.php`, `bot_api_helpers.php`, and `request.php`. They are interdependent — `successful.php` and `database_helpers_2.php` in particular share the `[wallet-done]` marker contract, so shipping one without the other breaks the "at most once" wallet deduction. When a merchant reports a delivery problem, first diff their files against these; several reported "bugs" turned out to be a partial install.
  - These files carry hard-won fixes for a specific failure mode: on busy shared hosts the delivery process dies mid-flight, leaving the service created in the VPN panel but nothing recorded in the bot. The defences layered in are (a) a fast blur path in `bot_api_helpers.php`, (b) atomic wallet deduction, (c) a rescue loop that only claims an order once `invoice.Status='active'` proves delivery actually happened, (d) a delivery retry on later callbacks, and (e) `request.php` releasing both DB connections around panel HTTP calls so they don't hold a `max_user_connections` slot for the 5–10s the panel takes.
  - **The rescue loop only ever scans new-purchase orders** (`Payment_report.id_invoice LIKE 'getconfigafterpay|%'`). Renewals/top-ups (تمدید/افزایش) are deliberately out of scope: the only "was it really delivered?" signal available is an `invoice` row reaching `Status='active'`, and for a renewal the service is already active, so that check returns true no matter what — neither confirmable nor refutable. An earlier version treated that as a third "unknown" state and alerted the admin, which fired a false "سرویس تحویل نشده — نیاز به رسیدگی دستی" on *every healthy renewal*. Don't reintroduce an unknown branch; widen the scan only if a per-order delivery signal is found first.
  - **Two markers, both on `Payment_report.dec_not_confirmed`, and the distinction matters.** `[wallet-done]` is written only inside the new-purchase wallet-deduction block in `database_helpers_2.php` and means "the at-most-once wallet deduction happened". `[cb-done]` is written in `successful.php` right after `DirectPayment()` returns and means "this callback ran to completion", for *every* order type. The rescue loop skips rows carrying either. Before `[cb-done]` existed, any order type that never reaches the wallet block stayed permanently unmarked, so the loop kept rediscovering long-completed orders and sending "🛟 گزارش بازیابی‌شده" hours after a perfectly successful payment. Any new delivery path must write `[cb-done]` on success.
  - **Two timing rules that are easy to get wrong and have each caused a real incident.** The gateway calls the callback twice — a server-to-server POST that performs delivery, then the customer's browser GET a few seconds later — while delivery itself takes 5–10s. So any code that scans for "incomplete" orders must exclude both the current request's own `id_order` *and* anything younger than a couple of minutes, or it will act on an order that is still being delivered. The rescue loop fails **open** on an unparseable `Payment_report.time` (a missed rescue is recoverable); the retry path fails **closed** (a double delivery is not).
  - `payment/ZarinPay/cubepay-callback.log` and `cubepay-create.log` are written by these files and are the fastest way to diagnose a merchant issue — ask for them before reading any code. If they show a step starting and never finishing with no `FATAL` line, suspect the host: check the bot's own `error_log` for `max_user_connections`. `integrations/mirzabot-ready-files/` used to follow the same pattern but CubePay is now merged upstream into Mirzabot directly (mahdiMGF2/mirzabot PR #75) — that directory is just a short pointer/status doc now, not a file-replacement guide.

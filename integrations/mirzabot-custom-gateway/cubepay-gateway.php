...
/**
 * توکنِ API رباتِ میرزای خودتان: دستورِ /token2 را داخلِ «همین رباتِ میرزا»
 * بزنید و توکنی که می‌دهد را اینجا بگذارید.
 * ⚠️ این توکنِ رباتِ میرزای خودتان است — نه رباتِ CubePay، و نه هیچ آیدیِ
 * عددی. اگر مقدارِ اشتباه بگذارید، پرداخت تایید می‌شود ولی سرویس تحویل
 * داده نمی‌شود.
 */
...
function handle_callback(array $in): void
{
    ...
    // 🔒 هرگز به خودِ کال‌بک اعتماد نکن — یا امضای HMAC رو چک کن (VIP)، یا از CubePay بپرس.
    if (!cg_is_really_paid($orderId, $authority, $in)) {
 ...
 */
function cg_is_really_paid(string $orderId, string $authority, array $in = []): bool
{
    // 🔐 مسیرِ VIP: کال‌بکِ VIP یک امضای HMAC (sig) داره که با توکنِ VIP
    // خودمون (CUBEPAY_TOKEN) ساخته شده. چون فقط CubePay این توکن رو داره،
    // امضای درست یعنی «قطعاً از CubePay و قطعاً پرداخت‌شده» — پس نیازی به
    // استعلامِ دوباره نیست (دقیقاً مثلِ فایلِ آماده‌ی فاکسیما). سفارشِ VIP
    // اصلاً authority نداره و تو check-order-status هم به‌شکلِ عادی نیست،
    // پس این تنها راهِ درستِ تاییدشه.
    $sig = (string) ($in['sig'] ?? '');
    if ($sig !== '') {
        $sigOrderId = $orderId !== '' ? $orderId : (string) ($in['order_id'] ?? '');
        $sigStatus  = (string) ($in['status'] ?? '');
        // مبلغِ امضاشده تومان است؛ فرستنده هم 'amount_toman' هم 'amount' رو با همین مقدار می‌فرسته.
        $sigAmount  = (string) ($in['amount_toman'] ?? ($in['amount'] ?? ''));
        $expected   = hash_hmac('sha256', $sigOrderId . '|' . $sigStatus . '|' . $sigAmount, CUBEPAY_TOKEN);
        if (hash_equals($expected, $sig) && $sigStatus === 'paid') {
            return true;
        }
        cg_log("VIP sig present but invalid/not-paid for order={$sigOrderId} status={$sigStatus}");
        // امضا خوند نشد → می‌افتیم رو مسیرهای عادیِ پایین (شاید کارتی/کریپتو بوده).
    }

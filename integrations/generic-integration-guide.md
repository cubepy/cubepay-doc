...
> 🐍 **کدتون PHP نیست؟** نمونه‌های این صفحه PHP هستن، ولی منطقش در هر زبانی یکیه. برای پایتون یه فایلِ کاملِ آماده داریم که همین مسیر رو از اول تا آخر نشون می‌ده: [`docs/examples/python-example.py`](../docs/examples/python-example.py). برای بقیه‌ی زبان‌ها [`docs/examples/`](../docs/examples/).

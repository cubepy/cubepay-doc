...
| کلیدِ امضای کال‌بک | توکنِ API شما | **همان توکنِ API عادیِ شما** (نه توکنِ VIP) |

> 📌 نکته‌ای که راحت اشتباه می‌شود: امضای `sig` در کال‌بکِ VIP هم با **توکنِ عادیِ** شما ساخته می‌شود، نه با توکنِ `vip_`. پس کدِ اعتبارسنجیِ فعلی‌تان بدونِ تغییر کار می‌کند.

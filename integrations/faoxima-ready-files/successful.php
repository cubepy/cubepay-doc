...
// ⬇️ کال‌بکِ پرداختِ کریپتویی CubePay — برخلاف کارتی، authority نداره؛
...
if (!$isCryptoCallback && ($authority === null || $invoiceId === null)) {
...
try {
    ...
    if (empty($token) || $token === '0') {
        throw new Exception('توکن درگاه تنظیم نشده است.');
    }
    ...
    if ($isCryptoCallback) {
        // ============================================================
        //  مسیر ۱ — پرداخت کریپتویی: تاییدِ اصالت با امضای HMAC
        //  (نیازی به verify-payment نیست؛ خودِ امضا تاییدیه‌ی نهاییه)
        // ============================================================
        $expectedSig = hash_hmac(
            'sha256',
            $invoiceId . '|' . $cryptoStatus . '|' . (string) $cryptoAmount,
            (string) $token
        );

        if (!hash_equals($expectedSig, (string) $cryptoSig)) {
            throw new Exception('امضای این کال‌بک معتبر نیست.');
        }

        if ($cryptoStatus !== 'paid') {
            throw new Exception($cryptoStatus === 'expired'
                ? 'مهلت این پرداخت تمام شده است.'
                : 'پرداخت انجام نشد.');
        }

        $result = ['success' => true, 'status' => 'verified'];
    } else {
        // ============================================================
        //  مسیر ۲ — پرداخت کارت‌به‌کارت (دقیقاً مثل قبل، بدون تغییر)
        // ============================================================
    ...
    // [FIX] DirectPayment() (called a few lines below) reads the "successful purchase"
    // caption template from the global $datatextbot. This file never populated it, so
    // DirectPayment() built the delivery caption from an undefined array — the user
    // ended up with a bare QR photo and no success text, no username/service info, and
    // no subscription link. The older payment/ZarinPay/successful1.php did this correctly;
    // restoring the same block here.
    global $datatextbot;
    ...
    if ($paymentReport['payment_Status'] !== 'paid') {
        ...
        if ($atomic->rowCount() >= 1) {
            ...
            if (!empty($setting['Channel_Report'])) {
                ...
                $reportLines[] = 'روش پرداخت : ' . ($isCryptoCallback ? 'کیوب‌پی (ارز دیجیتال)' : 'کیوب‌پی (کارت‌به‌کارت)');

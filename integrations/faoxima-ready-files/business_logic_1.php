 ...
 *
 * 📌 معمولاً لازم نیست این را دست بزنید: این تنظیم از رباتِ CubePay خوانده
 * می‌شود — «🏪 فروشگاه من → 💳 روش‌های پرداخت → 🤖 نمایش کارت در ربات».
 * هر چه آنجا انتخاب کنید، همین‌جا هم اعمال می‌شود.
 *
 * این ثابت فقط یک «اجبارِ محلی» است:
...
function createPayZarinpey($price, $order_id, $userId)
{
    ...
    $showCardInBot = CUBEPAY_SHOW_CARD_IN_BOT || !empty($result['show_card_in_bot']);
    if ($showCardInBot
        && !empty($userId)

...
# 🧩 معماری «CubePay VIP» (تسویه توسط CubePay / Managed Settlement) — پیش‌نویس RFC

> ⚠️ **وضعیت این سند: پیش‌نویس معماری، هنوز پیاده‌سازی نشده.** این یک RFC داخلیه، نه مستند یک ویژگیِ زنده — هیچ endpoint، جدول یا تنظیمی که اینجا اومده روی `cubevps.ir` وجود نداره. طبق [`CONTRIBUTING.md`](../CONTRIBUTING.md) («برای تغییرات بزرگ، قبلش یک Issue باز کنید») پیشنهاد می‌شه قبل از شروع پیاده‌سازی، این سند به‌عنوان مبنای بحث روی یک Issue قرار بگیره.
>
...
**تفاوت بنیادی با مسیر فعلی:** در مسیر SMS Forwarder، CubePay فقط «تأییدکننده»ست — پول همیشه مستقیم به کارت خودِ فروشنده می‌ره. در این ماژول، CubePay «جمع‌کننده و امانت‌دار» (custodian) پوله. این یک تغییر نقش قابل‌توجهه و باید به همین چشم (ریسک، مسئولیت، الزامات احراز هویت) بهش نگاه بشه — به بخش [نکات حقوقی و ریسک](#️-نکات-حقوقی-و-ریسک-عملیاتی) توجه کنید.
...
این ماژول جدید با این فرض کار نمی‌کنه (پایین، بخش [نکتهٔ حیاتیِ نقدینگی](#-نکتهٔ-حیاتی-که-باید-قبل-از-پیاده‌سازی-حل-بشه-نقدینگی-ارزی) رو ببینید) پس به‌جای import مستقیمِ `nowpayments-lib.php`، پیشنهاد می‌شه:
...
```mermaid
flowchart TD
    A[فروشنده: درخواست فعال‌سازی «تسویه توسط CubePay»] -->|بررسی دستی| B{ادمین تأیید می‌کند؟}
    B -->|رد| A
    B -->|تأیید| C[ساخت پروفایل تسویهٔ فروشنده: سطح، سقف‌ها، کارمزد، زمان‌بندی]
...
پیشنهاد برای فاز اول: این بخش را *خارج از دامنه‌ی این ماژول* نگه دارید و به‌عنوان یک فرآیند عملیاتیِ دستی (نه خودکار) مدیریت کنید — یعنی ادمین باید از پنل «موجودی master account» را رصد کند و طبق نیاز، خودش دستی خزانه را از یک صرافی شارژ کند؛ در غیر این‌صورت هر بار حجمِ تسویه از موجودی واقعیِ master account روی NOWPayments بیشتر بشه، درخواست‌های برداشت به‌طور غیرمنتظره `Failed` می‌شوند. این ریسک باید قبل از فعال‌سازیِ اولین فروشندهٔ واقعی به تیم گزارش شود؛ «برداشت خودکار به آدرس فروشنده» که در درخواست اولیه آمده، مشروط به حل همین نقدینگی است.
...
| بخش | وضعیت |
|---|---|
| API ساخت/استعلام فاکتور، Ledger، ضدتکرار، سقف‌ها، پنل ادمین/فروشنده | 🆕 طراحی جدید، صفر پیاده‌سازی |
| اعتبارسنجی امضای IPN، TOTP، کش JWT/نرخ، الگوی retry کال‌بک | ♻️ بازاستفاده مستقیم از `crypto/nowpayments-lib.php` از طریق adapter |
| مسیر SMS Forwarder / کارت‌به‌کارت فعلی | ✅ بدون تغییر |
| تأمین نقدینگیِ خزانه‌ی ارزی برای master account | ❗ باز — نیازمند تصمیم عملیاتی قبل از فعال‌سازی فروشنده‌ی واقعی |

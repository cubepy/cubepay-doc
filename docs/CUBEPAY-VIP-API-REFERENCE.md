...
| اشتراکِ ماهانه‌ی VIP | **۱٬۰۰۰٬۰۰۰ تومان / ۱ ماه** |
...
| حداقلِ مبلغِ هر برداشت | **۵۰۰٬۰۰۰ تومان** |
...
| `customer_fee_share` | number | ❌ | سهمِ **مشتری** از کارمزدِ درصدی، بین `0` تا `1` — پیش‌فرض `0` (کارمزد کامل با شما). `0.5` = نصف‌نصف، `1` = کارمزد کامل با مشتری. مبلغِ پرداختیِ مشتری خودکار بزرگ‌تر می‌شه (gross-up با رُند به بالا) طوری که بعد از کسرِ کارمزد، دریافتیِ شما از قیمتِ پایه کمتر نشه |
...
اگه `customer_fee_share` فرستاده باشید، پاسخ دو فیلدِ اضافه هم داره و `amount_toman` مبلغِ gross-up شده‌ست (چیزی که مشتری واقعاً می‌پردازه):
...
> ⚠️ با `customer_fee_share`، توی کال‌بک و `check-order-status.php` هم `price_toman` جدا میاد — کیف‌پول/سفارشِ مشتری‌تون رو با **`price_toman`** (قیمتِ خودتان) شارژ کنید، نه `amount_toman`. سقف‌ها (هر تراکنش/روزانه/ماهانه) به مبلغِ ناخالص می‌خورن. امضای `sig` همچنان فقط روی `amount_toman` ساخته می‌شه.

...
> ⚠️ **اگر هنوز از endpoint قدیمیِ کارت‌به‌کارت** (`/smspay/api/create-payment.php`) **استفاده می‌کنید:** آن مسیر توکنِ VIP را نمی‌شناسد و خطای `توکن نامعتبر است.` می‌گیرید. برای VIP باید آدرس را به `POST /pay/create-order.php` تغییر دهید و مبلغ را **به تومان** در `price_amount` بفرستید — endpoint قدیمی مبلغ را به **ریال** در فیلد `amount` می‌گرفت، این تفاوت را جا نیندازید.
...
| مورد | مقدار پیش‌فرض |
|---|---|
| کارمزدِ CubePay روی هر فاکتور | **۱۰٪** |
...
| کارمزد | ۳٪ | ۱۰٪ (قابلِ تنظیم برای هر فروشنده) |

...
🎛 **`show_card_in_bot` یعنی چی؟** خواسته‌ی خودتونه که تو ربات تنظیم کردید — «🏪 فروشگاه من → 💳 روش‌های پرداخت → 🤖 نمایش کارت در ربات». ما کاری نمی‌کنیم، فقط این خواسته رو به کدِ شما اعلام می‌کنیم: اگه `true` بود یعنی «کارت رو تو ربات هم نشون بده». فایل‌های آماده‌ی ما خودشون ازش پیروی می‌کنن؛ اگه کدِ خودتون رو نوشتید، هرطور خواستید ازش استفاده کنید (یا کلاً نادیده بگیرید). 👉 اینکه بسته به نوعِ اتصال‌تون دقیقاً باید چی‌کار کنید: [نمایش کارت داخل ربات](./CARD-IN-BOT.md).

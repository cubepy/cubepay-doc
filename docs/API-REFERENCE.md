...
| `redirect_after_payment` | bool | ❌ | اگر `false` بدهید، بعد از تاییدِ پرداخت مرورگرِ مشتری به `callback_url` هدایت **نمی‌شود** — فقط پیامِ «پرداخت تایید شد» را در همان صفحه می‌بیند. `callback_url` سرور-به-سرور همچنان و بدون تغییر صدا زده می‌شود. برای یکپارچه‌سازی‌های ربات‌محور (که نتیجه را از طریق پیام تلگرام، نه ریدایرکتِ مرورگر، به مشتری می‌رسانند) `false` بدهید تا دامنه‌ی شما هیچ‌وقت به مشتری نشان داده نشود. **اگر این پارامتر را نفرستید**، تنظیمِ حسابِ خودتان اعمال می‌شود — «🏪 فروشگاه من → 💳 روش‌های پرداخت → 🔀 ریدایرکت بعد از پرداخت» در ربات. مقدارِ صریحِ این پارامتر همیشه بر تنظیمِ حساب اولویت دارد. حساب‌های جدید با این تنظیم **خاموش** شروع می‌کنند (چون بیشترِ اتصال‌ها ربات‌محورند)؛ حساب‌هایی که از قبل وجود داشته‌اند بدون تغییر **روشن** مانده‌اند. |
...
🤖 **`card` برای چیه؟** اگه فروش‌تون از طریق رباته و نمی‌خواید مشتری رو به صفحه‌ی وب بفرستید، با همین فیلدها می‌تونید پرداخت رو **مستقیم داخل ربات خودتون** نشون بدید (شماره کارت، نام صاحب کارت، مبلغ دقیق، مهلت) — نمونه‌ی آماده در [راهنمای اتصال عمومی](../integrations/generic-integration-guide.md#-نمایش-پرداخت-داخل-ربات-بدون-صفحهی-وب).

⚠️ **کارت رو از قبل ذخیره و تکرار نکنید:** اگر «چرخش کارت» را روشن کرده باشید، هر فاکتور ممکن است کارتِ متفاوتی بگیرد. همیشه همان کارتی را نشان دهید که در پاسخِ *همین* فاکتور آمده است.

...
| `redirect_after_payment` | bool | ❌ | پیش‌فرض `true`. اگر `false` بدهید، بعد از تاییدِ پرداخت مرورگرِ مشتری به `callback_url` هدایت **نمی‌شود** — فقط پیامِ «پرداخت تایید شد» را در همان صفحه می‌بیند. `callback_url` سرور-به-سرور همچنان و بدون تغییر صدا زده می‌شود. برای یکپارچه‌سازی‌های ربات‌محور (که نتیجه را از طریق پیام تلگرام، نه ریدایرکتِ مرورگر، به مشتری می‌رسانند) `false` بدهید تا دامنه‌ی شما هیچ‌وقت به مشتری نشان داده نشود. |
